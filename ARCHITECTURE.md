...
| `src/main.rs` | 入口：`mod` 声明 + `run()` 装配（`AppState` 初始化、插件注册、`generate_handler!`） |
| `src/state.rs` | `AppState` / `MctsTreeHandle` / `OpponentType` / `GameState` / `StepResult`+ 锁辅助 `lock()`（中毒恢复）+ `extract_game_state`（含**锁顺序约定**文档） |
| `src/game_cmds.rs` | 对局命令：`reset_game` / `step_game` / `bot_move` / `get_game_state` / `get_opponent_type` / `get_move_action` |
| `src/engine_cmds.rs` | 引擎参数命令：`set_mcts_iterations` / `set_engine_budget` / `set_nnue_depth` / `set_nnue_budget` |
| `src/model_cmds.rs` | 模型命令：`get_capabilities` / `list_models` / `open_models_dir` / `load_model`（含 `.pt`/`.onnx`/`.nnue` 分派） |
| `src/mcts_view.rs` | 搜索树可视化：`mcts_get_root` / `mcts_get_children` / `mcts_get_node_detail` / `mcts_get_node_board` / `mcts_search` + 树 DTO 与只读投影 |
...
- 2026-09-15：同步 `banqi-core` 的 MCTS 评估契约变更：`mcts_view.rs::mcts_search_blocking` 的两次 `mcts.run()` 改为 `Result` 处理（评估失败经 `to_string()` 变为 command 错误返回前端，不再 panic）；`MctsDlPolicy` / `OnnxMctsPolicy` 的 `choose_action` 现返回 `Result`（gui 未直接调用）。
- 2026-09-15：质量重构：①`src/main.rs`（1044 行）拆为 `state.rs` / `game_cmds.rs` / `engine_cmds.rs` / `model_cmds.rs` / `mcts_view.rs`（`main.rs` 仅剩入口装配，86 行；单文件最大 301 行）；②全部 47 处 `.lock().unwrap()` 改为 `state::lock()`（锁中毒时 `into_inner()` 恢复，避免任一命令 panic 导致该锁永久污染、后端连锁不可用）；③在 `state.rs` 固化**锁顺序约定**（`game` → `opponent_type` → `mcts_num_simulations` → … → `mcts_tree`），经核查原代码各跨字段获取均为临时 guard 或同序，无实际颠倒；④`bot_move` 内部 `unreachable!()` 兜底改为返回 `None`（落到「AI 无棋可走」错误，不再 panic）；⑤清除 gui 侧 clippy 告警（`Copy` 类型 `.clone()`、`&vec![]`）。
...
- 2026-09-15：搜索树数值符号着色：价值类数值（树节点 `Q`、工具条根 `Q`、检视区 `Q`/`Q_hp`/`V 先验`）按符号着色（正=绿 `#1e7d43`，负=红 `#c0392b`，新增 `.val-pos`/`.val-neg`），不再只靠 `-` 号辨识；树节点统计 `N=`/`Q=` 拆为两个 `tspan`（`dx` 留间距）以便只给 `Q` 上色。
